...
class Checkpointer:
    ...
    def resume(self, f=None):
        ...
        self.logger.info("Loading checkpoint from {}".format(f))
        #使用_load_file方法实际从文件加载检查点。
        checkpoint = self._load_file(f)
        #使用_load_model方法加载模型状态。
        self._load_model(checkpoint)
    ...
    def _load_file(self, f):
        #torch.load函数用于加载保存的PyTorch模型或张量
        return torch.load(f, map_location=torch.device("cpu"))

    def _load_model(self, checkpoint, except_keys=None):
        #这行代码的作用是从 checkpoint 字典中移除键为 "model" 的项，并返回对应的值，
...
def strip_prefix_if_present(state_dict, prefix):
    ...
    #函数首先检查字典中所有的键是否都以给定的前缀 prefix 开头。如果不是，就原样返回输入的参数字典 state_dict。
    if not all(key.startswith(prefix) for key in keys):
        return state_dict
    #如果所有键都以指定前缀开头，就创建一个新的字典 stripped_state_dict，其中的键名是去除了前缀的版本，而值保持不变。
    stripped_state_dict = OrderedDict()
    for key, value in state_dict.items():
        stripped_state_dict[key.replace(prefix, "")] = value
    return stripped_state_dict

#加载模型参数（state_dict）的函数
def load_state_dict(model, loaded_state_dict, except_keys=None):
    ...
    #如果保存的模型是在DataParallel或DistributedDataParallel中进行了序列化，它可能有额外的前缀，如"module."。这一步操作去除了
    loaded_state_dict = strip_prefix_if_present(loaded_state_dict, prefix="module.")
    # 这个函数是对两个模型参数字典进行对齐和更新的操作。主要目的是确保模型参数的形状匹配，并将loaded_state_dict中的参数值更新到model_state_dict中。
